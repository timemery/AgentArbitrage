...
def recalculate_deals():
    ...
    try:
        ...
        required_columns_map = {
            ...
            "Title": "Title",
            "Categories - Sub": "Categories___Sub",
        ...
        for original_name, alias in required_columns_map.items():
            sanitized_original = _sanitize_col_name(original_name)
        ...
        for i, deal_data in enumerate(deals_to_refresh):
            ...
            try:
                detailed_season = classify_seasonality(deal_data.get('Title', ''), deal_data.get('Categories___Sub', ''), deal_data.get('Manufacturer', ''), xai_api_key=XAI_API_KEY)
                sells_period = get_sells_period(detailed_season)
                row_updates['Detailed_Seasonality'] = "None" if detailed_season == "Year-round" else detailed_season
                row_updates['Sells'] = "All Year" if detailed_season == "Year-round" else sells_period
        ...
        for row in all_rows_to_update:
            try:
                ...
                sanitized_update_dict = {_sanitize_col_name(k): v for k, v in update_dict.items()}
