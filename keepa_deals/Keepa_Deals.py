    ...
    try:
        ...
        for offset in range(0, total_deals, BATCH_SIZE):
            ...
            if not product_data or 'products' not in product_data:
                logger.warning(f"Could not fetch product data for batch starting at offset {offset}. Skipping.")
                continue
            ...
            for asin, product in products_map.items():
                ...
                try:
                    ...
                    logger.error(f"Failed to update DB for ASIN {asin}: {e}")

            conn.commit()
            processed_count = offset + len(asins_to_process)
            logger.info(f"Batch complete. Total processed: {processed_count}/{total_deals}")
