...
- **`_merge_db_keyed` skips no-data sentinels (`-`, `''`, `N/A`, `None`) rather than writing them.** Field functions report failure in band, under their normal key, so a caller that writes unconditionally replaces a good stored value with nothing. The row being merged into is an existing DB row, so the stored value is always the better answer. Do NOT rewrite that check as a falsiness test: `0` and `'0'` are real readings and must overwrite. See "LIGHTWEIGHT PRESERVATION RULE" in `Data_Logic.md`.
