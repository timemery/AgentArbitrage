...
- **The AI Reasonableness Check fails CLOSED (Trello #144).** Daily cap reached or xAI error ⇒ `None` (unverifiable), not `True`. The price is withheld and `_process_single_deal` writes `Pricing_Logic_Version` **NULL**, read from the memoised analysis that produced `List at` (`_get_analysis`), not from the second `analyze_sales_performance` call beside it — that call's check can succeed where the first failed. Pinned by `tests/test_xai_fail_closed.py`.
  ...
  - A missing `XAI_TOKEN` still skips the check and passes the price, as before.
