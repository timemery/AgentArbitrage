...
- **`repair_pricing.py` stops before the xAI daily cap.** Past the cap, `_query_xai_for_reasonableness` returns `True` rather than failing or skipping (`stable_calculations.py:76`), so a price would be accepted unchecked **and** stamped with the current version — dropping it out of the repair predicate permanently. The sweep therefore stops while the check still works (`--xai-headroom`, default 50) and is resumed after the daily reset. **The xAI cap, not Keepa tokens, bounds the sweep**: ~1.7 calls per row against 1000/day shared with ingestion is ~500-600 rows/day.
