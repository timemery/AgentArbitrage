...
- **September 2026 Addendum 2 — the xAI Sales Rescue is REMOVED (2026-09-16, Trello #141).** A third source of unverified prices survived both removals above: `infer_sale_events` called `infer_sales_with_xai` on **both** of its zero-sale branches and returned the model's events verbatim. A model-asserted sale is **not** a true inferred sale. It is not an offer drop correlated with a rank drop; nothing checked that its price ever appeared in the Keepa history at all. It is now gone, and **zero confirmed sales is a final answer**.
  - It returned **before** the IQR filter, and before the `price <= 0` and NaN guards in the correlation loop. Its only price check was a truthiness test, which admits a negative price.
  - The price-association fix of 2026-09-12 (PR #340) never applied to it — a rescued price is not read from `csv[1]`/`csv[2]`, it is asserted.
  - In production, 2026-08-28 to 2026-09-08, **every rescue returned exactly one sale**. At n=1 the Sparse Sales Rescue takes the median of one number, so `List at` and `1yr. Avg.` became the same single model-asserted figure — and because a rescued event always landed inside 365 days, such rows **always** cleared the dashboard's data-completeness filter. Rescued deals were precisely the ones subscribers saw.
  - On the "offer drops that all failed correlation" branch it also corrupted `Deal Trust`: one model event over `total_offer_drops` read as **1/N**, a positive confidence score built from drops that had just failed. It now reads a truthful `0%`, and those drops still count in the denominator.
  - `keepa_deals/xai_sales_inference.py` still exists and still works. **Nothing in the live pipeline calls it.** Do not re-wire it into `infer_sale_events`. The absence is pinned by `tests/test_xai_rescue_excluded.py`.
  - This does **not** touch the AI Reasonableness Check on the pricing stage, which is a different mechanism and is unchanged.
...
- **Sparse Sales Rescue:** Median of inferred sales (1-2 events) used as a "Sparse Rescue" price — represents true inferred sales. Skips the AI Reasonableness Check to prevent false negatives from missing context. **Since Pricing Logic Version 3 it is held to the thin-peak-season minimum (§7.15):** the 1-2 sales must be 2 distinct price points within one peak season (peak month ± 1, pooled across years). A sparse row below that — one sale, two sales in different seasons, or two sales priced by one point — is **persisted unpriced (`List_at` NULL), never deleted**, exactly like Zero Profit & Missing Data above, so the re-acquisition-loop protection still holds. **Those 1-2 events must be ALGORITHMIC** (offer drop correlated with rank drop). The xAI Sales Rescue that used to feed model-asserted events into this same branch was removed on 2026-09-16 — see §7.1, September 2026 Addendum 2.
...
- **Amazon Ceiling Check:** Applied on the **heavy path only**, inside `analyze_sales_performance` (`stable_calculations.py`): `List at` is capped at 90% of `min(Amazon current, avg180, avg365)`, with `current` counted only when today's month is the peak month (Pricing Logic Version 3, §7.15). **The lightweight-update clamp in `keepa_deals/processing.py` is OFF** — gated by `ENABLE_LIGHTWEIGHT_CEILING_CLAMP = False` pending a written spec for `List at` seasonal semantics (a peak price must survive the trough). It never fired in production before that gate either (it read a key the light-path row does not have). Do not turn it on casually. *(Corrected 2026-09-22: this entry used to say the light path clamps.)*
...
- **`repair_pricing.py` stops before the xAI daily cap.** Until Pricing Logic Version 3, past the cap `_query_xai_for_reasonableness` returned `True`, so a price was accepted unchecked **and** stamped current — dropping it out of the repair predicate permanently. It now fails **closed** (§7.15), which ends that, but the sweep still stops while the check works (`--xai-headroom`, default 50): past the cap every row would come back unpriced and hidden, for ~7 Keepa tokens each, and be paid for again on the retry. Resume after the daily reset. **The xAI cap, not Keepa tokens, bounds the sweep**: ~1.7 calls per row against 1000/day shared with ingestion is ~500-600 rows/day.
- **`repair_pricing.py` cannot recompute two columns.** `Deal_found` and `last_price_change` read keys that exist only on the **/deal feed object**, which the ingestor merges in at `smart_ingestor.py:573` but which cannot be fetched for an arbitrary ASIN. Their stored values are carried forward through an explicit two-column allowlist (`CARRY_FORWARD_COLUMNS`), established by auditing all 67 non-`None` `FUNCTION_LIST` entries. **Never widen that allowlist to a pricing column** — `List_at`, `1yr_Avg`, `Deal_Trust`, `Inferred_Sale_Count`, `Profit`, `Margin`, `Percent_Down` and the rest must be overwritten, including to NULL, or the script is an expensive no-op. It is not `_merge_db_keyed`, which guards every column and is the light path's helper.

### 7.15 Pricing Logic Version 3 (September 2026)

One `PRICING_LOGIC_VERSION` bump (2 → 3) covers every item below, so the ~8-day re-sweep is paid once. **Do not ship any one of them on its own branch.** Evidence and decisions: `Dev_Logs/2026-09-22_Prime_Picks_Guard_And_The_Thin_Peak_Month.md`.

- **The peak-season mode and median count DISTINCT PRICE POINTS, not sale events.** Each sale from `infer_sale_events` carries `price_point` = (series, timestamp of the change-log point that priced it). Two offer drops priced by one point are one asking price. Identity is the point's timestamp, **never price equality**. Pinned by `tests/test_distinct_price_points.py`.
- **Peak-window New cap.** `List at` ≤ median of per-peak-window New floors + $3.99 (`PEAK_NEW_CAP_ALLOWANCE_CENTS`). **Never today's New price** — the product buys at the trough and sells at the peak, so today's New offer is the buy side. No New price in the window ⇒ uncapped, recorded as unavailable, no fallback. Pinned by `tests/test_pricing_v3_caps.py`.
- **Amazon ceiling reads `stats.current` only when today's month is the peak month.** Both trailing averages stay. This change **may never ship without the cap**: removing a clamp can only raise prices.
- **The AI Reasonableness Check fails CLOSED (Trello #144).** No `XAI_TOKEN`, daily cap reached, or xAI error ⇒ `None` (unverifiable), not `True`. The price is withheld and `_process_single_deal` writes `Pricing_Logic_Version` **NULL**, read from the memoised analysis that produced `List at` (`_get_analysis`), not from the second `analyze_sales_performance` call beside it — that call's check can succeed where the first failed. Pinned by `tests/test_xai_fail_closed.py`.
  - **`repair_pricing.py` refuses to start without `XAI_TOKEN`** (exit 2, before preflight, backup or any fetch): otherwise it would write every checked row unpriced and hidden. Pinned by `tests/test_repair_pricing.py::ItRefusesToRunWithoutAnXaiKey`.
  - **Only the repair sweep retries such a row.** The ingestor routes existing ASINs to the light path, and the Zombie Data Defense re-fetch is disabled, so nothing else re-prices it. In the sweep it sorts **last** (tier 2, not priced).
  - **A missing `XAI_TOKEN` fails closed too** (owner decision, 2026-09-22): a silent pass on a missing key is the same failure as #144. Consequence: any environment without the key — a dev box, a misconfigured worker — prices nothing that reaches the check. The Sparse Sales Rescue and Amazon-ceiling-clamped prices skip the check, so they still price.
- **The thin peak season is not priced.** The peak season is the peak month ± 1 month (`PEAK_SEASON_HALF_WIDTH_MONTHS`), pooled across every year; `List at` is estimated over it. Fewer than **2 distinct price points** (`PEAK_SEASON_MIN_PRICE_POINTS`) ⇒ `List_at` NULL, row persisted and hidden, **never deleted** (§7.8), stamped with the current version (it is a real answer, not a retry), no AI call. Applies to the Sparse Sales Rescue too. Owner decision 2026-09-22 from `audit_list_at_sources.py` section (d) on 100 random visible rows: hides 28 (6 sparse); the single month alone would have hidden 65. Pinned by `tests/test_thin_peak_season.py`.
  - **Superseded in Pricing Logic Version 4 (§7.16):** the peak season is now chosen by pooled support rather than centred on the single highest month, and a thin row is written with a **NULL** version so the sweep re-evaluates it (#152), not stamped current.

### 7.16 Pricing Logic Version 4 (September 2026)

One bump (3 → 4). Measured before building, on the 63 rows v3 withheld (`audit_list_at_sources.py --hidden-v3`): 31 thin (27 of them **lone spikes** — one isolated high month won the v3 vote and its season held one point), 28 AI "No", 1 over $1,500. Owner decisions 2026-09-23.

- **Peak season by pooled support.** Every peak-month ± `PEAK_SEASON_HALF_WIDTH_MONTHS` window, pooled across years, holding ≥ `PEAK_SEASON_MIN_PRICE_POINTS` (2) distinct points is eligible; the **highest median** wins. Ties: more points; more distinct points in the centre month itself (so a one-month season is centred on that month, not an empty neighbour); earlier month. None eligible ⇒ thin, hidden. `_best_peak_window` in `stable_calculations.py`. Measured: 61 of the 63 withheld rows priced.
- **2× 1yr-median cap.** The best-median window is still a maximum over thin estimates: 15 of 61 came out above 2× their own 1yr median, the worst 5–20×. `List at` ≤ `PEAK_MEDIAN_CAP_RATIO` (2.0) × the median of inferred sale EVENTS in the last 365 days (a median, because a mean is inflated by the same lone spikes). **No sale in the last year ⇒ no median cap**; the AI check still applies. It is derived only from true inferred sales, so it is not a §7.1 fallback. Owner chose k=2 from 1.5/2/3 (caps 22/15/11 of 61).
- **Cap order:** branch price → peak-window New cap → median cap → Amazon ceiling → $1,500 ceiling. Each only lowers the price.
- **AI check skipped at ≤ `AI_SKIP_MEDIAN_RATIO` (1.25) × the 1yr median.** The check is never shown the book's own sale prices and rejected prices at their own 1yr average (1936164116: $398.99 vs $398.99). The skip **takes precedence over the 3×-of-current-used rule** (that compares with today's trough-time Used price; this with what the book sold for). Above it the check runs, fail-closed (§7.15). Measured: 35 of 61 priced rows skip.
- **Every withheld price records why:** `withheld_reason` on the analysis — `thin`, `ai_rejected`, `unverifiable`, `over_1500`, `no_sales`. `repair_pricing.py` logs it on each row's line (`| withheld: …`), read from the memoised analysis, **never recomputed** (a recompute could spend another xAI call). **No DB column** (owner decision).
- **#152: thin rows are written with a NULL `Pricing_Logic_Version`,** so the repair sweep re-evaluates them as they gain sales; they sort last (tier 2). ~7 Keepa tokens per thin row per sweep, no xAI call. AI-rejected and over-$1,500 rows are still stamped current.
- **The audit reads production.** Its `v4_*` columns come off production's analysis (`peak_window`, `peak_window_points`, `one_year_median_cents`, `ai_skipped_by_median`, `withheld_reason`); there is no private copy of these rules.
- **Pinned by** `tests/test_pricing_v4.py`, `tests/test_thin_peak_season.py`, `tests/test_repair_pricing.py::TheWithheldReasonIsLogged`.

### 7.14 Stale Pricing Is Never a Prime Pick (September 2026)

**No row with stale pricing is ever shown as a Prime Pick, on any path.** Agent's Choice is the one surface that presents a deal as a recommendation, so it is held to the repair sweep's own definition of "current" — §7.13's NULL rule, read in the other direction.

- **One predicate, three call sites.** `CURRENT_PRICING_PREDICATE` in `keepa_deals/pricing_version.py` is `'NOT ' + STALE_PRICING_PREDICATE`, **derived, not restated**. If the display rule and the scheduling rule ever drift, the sweep repairs one set of rows while the dashboard hides a different one. Do not write a second copy; `tests/test_prime_picks_stale_pricing.py::TheRuleIsDefinedOnlyOnce` fails the build if you do.
- **Why Pass 1 alone is not enough.** Two paths through `generate_prime_picks` return without writing to `prime_picks`: Pass 1 finding no eligible candidates, and Pass 2 failing or selecting nothing. Both preserve the previous cache by design (§7.10), so a pick chosen before the filter existed survives them all. Hence `prune_stale_priced_picks`, run **before Pass 1 on every invocation** — staleness is not something a row acquires on its own, but bumping `PRICING_LOGIC_VERSION` re-stales the entire table at once, and that bump is the documented way to schedule the next sweep.
- **And why the eviction alone is not enough either.** It runs only when the task runs (4-hourly, chained after the Janitor). The Agent's Choice branch of `/api/deals` therefore applies the same predicate at read time, which is what makes the invariant true at the moment of display rather than at the moment of the last successful run.
- **Agent's Choice ONLY.** Do NOT extend this to the main dashboard grid. Suppressing stale-priced rows there would empty it while a sweep is in flight — 232 rows were still stale when the 2026-09-21 run ended — and that is an owner decision, not a consequence of this one.
- **The evidence.** 2026-09-22, live: Prime Picks rebuilt at 12:00 UTC with four picks; pick #4, ASIN `1418548839`, `List_at` 219.66, carried `Pricing_Logic_Version` NULL. The sweep had skipped it ("heavy processing returned nothing (usually: no used offer)") and a skipped row keeps its old prices indefinitely, so the price on screen was written by the pre-2026-09-12 logic the sweep exists to replace. 231 rows were skipped in that run.
- **Guarded by:** `tests/test_prime_picks_stale_pricing.py`.
...
- **Agent's Choice (Prime Picks) Filter Fix:** Resolved a 504 Gateway Timeout issue caused by sending overly large payloads to the xAI API. The solution implements an async **Two-Pass Pipeline** in `keepa_deals/prime_picks_task.py`:
  ...
  - **Graceful Fallback:** If Pass 2 fails (e.g., due to xAI API errors), the system explicitly skips updating the cache, preserving the last known valid run instead of displaying an empty list or unfiltered candidates.
    - **Narrowed, September 2026: a preserved run may not preserve stale-priced picks.** `generate_prime_picks` calls `prune_stale_priced_picks` before Pass 1 on every invocation, so a preserved cache keeps its current-priced picks and loses the rest. This is required because the zero-candidate and Pass 2 failure paths both return without writing to `prime_picks` at all — without the eviction, a pick selected under superseded pricing logic survives every such run indefinitely. If that empties the cache, Agent's Choice shows nothing. See §7.14.
