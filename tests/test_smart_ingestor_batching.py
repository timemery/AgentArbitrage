...
class TestSmartIngestorBatching(unittest.TestCase):
    ...
    def test_batching_logic(self, mock_celery, mock_requeue, mock_create_table, mock_save_wm, mock_load_wm,
        ...
        # Check offers kwarg or positional arg
        # In smart_ingestor: fetch_current_stats_batch(..., offers=20)
        # offers is passed as kwarg 'offers' or pos? It's defined as pos in mock side_effect, but call might use keyword.
        # side_effect receives args/kwargs based on call.
        # But mock call_args stores them normalized? No.
        # We check call_args (tuple of args, kwargs).
        # smart_ingestor call: fetch_current_stats_batch(api_key, chunk_new_asins, days=365, offers=20)
        # Positional: api_key, chunk_new_asins
        # Keyword: days, offers
