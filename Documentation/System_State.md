...
**v4 columns.** Every row carries Pricing Logic Version 4's own record, read off production's analysis (no private copy): `v4_list_at`, `v4_window` and `v4_window_points` (the winning peak window), `v4_1yr_median`, `v4_skip_median` (AI check skipped at ≤ 1.25× that median) and `withheld_reason`. A `--hidden-v3` run ends with a `V4` block counting priced / AI-skipped / unpriced rows per group. The audit stubs the AI check, so a priced row that is not skipped still depends on an AI answer it cannot know.
