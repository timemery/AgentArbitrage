...
4.  **(d) The thin peak season (added for Pricing Logic Version 3).** For each candidate minimum in `PEAK_SEASON_MIN_CANDIDATES` (2, 3, 4), how many sampled rows that production prices today would be left unpriced, under two definitions of the season: the peak month alone pooled across years (what production groups on — `.dt.month`, so every September counts together), and the peak month ± `PEAK_WINDOW_HALF_WIDTH_MONTHS` (1) pooled across years, wrapping December into January. Counts are **distinct price points**, not sale events. The bracketed figure is how many of those rows are Sparse Sales Rescue rows (AGENTS.md §7.8), which have no peak month of their own; for them — and only them — the centre is mirrored from the normal branch's rule (month with the highest median, first on a tie) and the row records that it was. Per-row counts are in the detail file, so a different minimum or width can be re-scored from it without another run. Beside the distinct points it records the **sale events** in the peak month and in peak ± 1 (`season_sales_month`, `season_sales_window`), which is what separates a steady seller at one price (many sales, one point) from a lone spike (one sale).

**`--hidden-v3`: why the current logic withheld a price.** The pricing path does not record the reason: a thin peak season and an AI "No" both stamp the current `Pricing_Logic_Version` and leave `List_at` NULL, and neither reaches the sweep's log. `--hidden-v3` selects rows at the current version with `List_at` NULL or ≤ 0 and `Inferred_Sale_Count` ≥ 2 (one-sale rows are always thin), re-runs production with the AI check stubbed True, and ends stdout with a `WITHHELD SPLIT` line — thin (fewer than `PEAK_SEASON_MIN_PRICE_POINTS` distinct points in the peak season) vs AI No (not thin, prices once the check is stubbed) vs other — followed by the ASINs in the AI and other groups. The recompute uses today's history, so a row whose season has filled in since the sweep reads as AI No. Bounded by `--limit`:

```bash
sudo -u www-data venv/bin/python audit_list_at_sources.py --hidden-v3 --limit 100
```

**v4 candidate (measurement only, not production logic).** Every row also carries what the proposed Pricing Logic Version 4 rules would do: `v4_list_at` and `v4_window` — the peak chosen by pooled support (every peak month ± 1 window with at least `PEAK_SEASON_MIN_PRICE_POINTS` distinct points, highest median wins) then production's New cap, Amazon ceiling and $1,500 ceiling — and whether the AI check would be skipped because the price is ≤ `V4_AI_SKIP_RATIO` (1.25) × the last 365 days of inferred sales, under both the mean (`v4_skip_mean`) and the median (`v4_skip_median`). A `--hidden-v3` run ends with a `V4 CANDIDATE` block counting priced / skipped / unpriced rows per group. A priced row whose check is not skipped still depends on an AI answer the script cannot know.
