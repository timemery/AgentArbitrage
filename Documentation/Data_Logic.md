...
2.  **Seller & Price Analysis**:
    ...
    *   **When a lightweight fetch cannot compute a value, the stored value is kept and the dashboard shows the last good reading.** It is never replaced with a blank. This is the deliberate trade-off: a reading that is a few hours stale is more useful than no reading at all, and it is strictly better than the alternative, since a `NULL` `Sales_Rank_Current` also drops the row out of the **Max. Sales Rank** filter entirely.
    *   **Five columns are exposed to this**, all of them merged through `_merge_db_keyed`: `Sales_Rank_Current`, `Offers`, `Offers_180`, `Offers_365` and `last_price_change`. `All_in_Cost` and `Min_Listing_Price` are **not** — they are always computed floats with no sentinel path, and are written directly rather than merged.
    ...
    *   **What went wrong:** the September 2026 upsert fix correctly routed all seven values through `_merge_db_keyed`, which meant the Stale Rescue began *persisting* them — including the unconditional `-`. That replaced a good heavy-path timestamp with a dash on **1,948** rows, and nulled `Sales_Rank_Current` on 14 more (`clean_numeric_values` casts `-` to `int`, fails, and stores `NULL`). Before the fix the sentinel had been discarded along with the genuinely fresh values, which masked it.
