...
3.  **Inferred Sales (The Engine)**:
    ...
    *   **XAI Rescue (Hidden Sales) — REMOVED 2026-09-16.** If the standard mechanism found 0 confirmed sales, or no offer drops at all, it used to call xAI to identify "Hidden Sales" (rank drops without offer drops) and returned the model's events verbatim. **It no longer does. Zero confirmed sales is a final answer on both branches.** A model-asserted sale is not an offer drop correlated with a rank drop, nothing verified its price against the Keepa history, it returned *before* the IQR filter and the `price <= 0`/NaN guards, and the 2026-09-12 price association never applied to it. In production every rescue returned exactly one sale, which at n=1 became both `List at` and `1yr. Avg.` — and always cleared the dashboard filter. Owner decision, Trello #141; see `AGENTS.md` §7.1 (September 2026 Addendum 2). `keepa_deals/xai_sales_inference.py` still exists but nothing in the live pipeline calls it; the absence is pinned by `tests/test_xai_rescue_excluded.py`.
    *   **Price Association (Sept 2026):** The price attached to a confirmed sale is the last point in the matching series (`csv[1]` New, `csv[2]` Used) **strictly before** the offer drop, at **any** distance. *(Previously `merge_asof(direction='nearest')`, which stored the leftover asking price of a copy that did not sell — the series steps UP at the moment of the sale because it holds the LOWEST offer, not a per-copy price. Confirmed live on 5 of 7 sales across 3 ASINs.)*
        *   **There is deliberately no time threshold.** A 240h tolerance was proposed and rejected on real data (owner decision 2026-09-12): 4 of the 7 measured preceding-gaps exceeded it, median 252.1h, max 95.1 days. The series is a change-log, so a long gap means the lowest offer had not changed and the distant point is correct rather than stale. See `INFERRED_PRICE_LOGIC.md` §2b.1.
        *   **A confirmed drop loses its price only when no point precedes it at all** (0 of 7 on the live sample). The drop still counts in the `Deal Trust` denominator.
...
-   **`Inferred Sale Count`**:
    ...
    -   **Logic**: Count of the sane sale events the pricing branch used, post-IQR. *(This previously added "raw on the XAI-rescue path" — that path was removed on 2026-09-16 and there is now only one path. `0` means the correlation loop ran and found nothing.)*
...
-   **`Deal Trust` (Deal Trust)**:
    ...
    -   **Non-numeric state**: `'-'`, returned when `total_offer_drops == 0` — a history with no offer drop anywhere in 3 years, so there is no denominator to score. **`/api/deals` casts it with `CAST(REPLACE("Deal_Trust", '%', '') AS REAL)`, which yields `0.0`, so any non-zero Min. Deal Trust filter silently excludes those rows.** *(This state used to coincide with the XAI "no offer drops" rescue, where every sale came from the model. That rescue was removed on 2026-09-16; the sale list is now empty and the state means only what it says.)*
    -   **Offer drops that all fail correlation now read `0%`, and that is the point.** When drops exist but none correlates with a rank drop, the numerator is 0 and the denominator is those drops. Until 2026-09-16 the xAI rescue returned one model-asserted event over that same denominator, reading **1/N** — 50% on two failed drops, which cleared the Agent's Choice floor of 40. See `AGENTS.md` §7.1.
