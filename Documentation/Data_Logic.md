...
3.  **Inferred Sales (The Engine)**:
    ...
    *   **Price Association (Sept 2026):** The price attached to a confirmed sale is the last point in the matching series (`csv[1]` New, `csv[2]` Used) **strictly before** the offer drop, and only if it is no older than `PRICE_ASSOCIATION_TOLERANCE_HOURS` (**240h**). Otherwise **no price is attached and the sale is discarded**; the offer drop still counts in the `Deal Trust` denominator. *(Previously `merge_asof(direction='nearest')` with no tolerance, which stored the leftover asking price of a copy that did not sell — the series steps UP at the moment of the sale because it holds the LOWEST offer, not a per-copy price. Confirmed live on 5 of 7 sales across 3 ASINs. See `INFERRED_PRICE_LOGIC.md` §2b.)*
