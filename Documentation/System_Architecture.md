...
*   **Mechanism:**
    ...
    3.  **Decoupled Batching Strategy:**
        ...
        *   **Stage 1: Peek (Discovery):** Fetches lightweight stats for **50 ASINs** (`SCAN_BATCH_SIZE`) at once at full refill rate.
            *   **Dynamic Scaling (corrected 2026-09-12 — this entry previously stated the wrong tiers in both directions).** The batch shrinks with the token refill rate Keepa reports, so one peek stays inside a refillable budget:

                | refill rate | peek batch |
                | :--- | :--- |
                | < 10 / min | **1** (peek 2 + commit 20 = 22 tokens, inside the 40-token burst) |
                | < 20 / min | **20** |
                | < 30 / min | **15** (~115 tokens; a 50-ASIN peek at `days=365, offers=20` costs ~386 and drives deep deficit) |
                | >= 30 / min | **50** |

                The **15**-cap is the tier production actually runs in: the live Keepa plan reports **25/min**. Commit batching is *not* rate-scaled — `COMMIT_BATCH_SIZE` is a flat **5**. Guarded by `tests/test_smart_ingestor_batching.py`.
        ...
        *   **Stage 1.5: XAI Rescue — REMOVED 2026-09-16.** If initial analysis found 0 confirmed sales, or no offer drops at all, the system used to call **xAI** to identify "Hidden Sales" (rank drops without offer drops) and fed the model's events straight into pricing. **It no longer does.** Zero confirmed sales is now a final answer: the deal is persisted with NULL `List_at`/`1yr_Avg` and `Inferred_Sale_Count` 0, and filtered off the dashboard. Owner decision, Trello #141 — a model-asserted sale is not a true inferred sale, it bypassed the IQR filter and the price guards, and in production every rescue returned exactly one event which became both `List at` and `1yr. Avg.`. See `AGENTS.md` §7.1 and `INFERRED_PRICE_LOGIC.md` §2.5.
    ...
    5.  **Pricing Repair (`repair_pricing.py`, out-of-band):** the ONLY way an existing row reaches the heavy path. The Smart Ingestor routes purely on `asin in existing_asins_set`, so an ASIN already in the table always takes the light path — and the light path, the Stale Rescue and `recalculator.py` all leave `List_at`/`1yr_Avg` alone. A row written under superseded pricing logic therefore keeps its prices forever. The repair script selects `Pricing_Logic_Version IS NULL OR < PRICING_LOGIC_VERSION`, re-fetches through `_process_single_deal`, and upserts through the shared `upsert_deal_rows`. Run manually and detached; it shares the Redis token bucket but never takes the `smart_ingestor` lock. See `AGENTS.md` §7.13.
        *   **It is also the only retry for an unverifiable price (Pricing Logic Version 3).** When the AI Reasonableness Check cannot run (no API key, daily cap, xAI error) it fails closed: `_process_single_deal` persists the row with `List_at` NULL (hidden) and `Pricing_Logic_Version` NULL (stale). Nothing on the ingestion paths re-prices an existing row, so it waits for the next sweep, where it sorts last. See `AGENTS.md` §7.15.
        *   **And for a thin peak season (Pricing Logic Version 4, #152).** Thin rows are written with a NULL version too, so each sweep re-evaluates them as they gain sales (~7 Keepa tokens each, no xAI call). Every withheld row's log line names its reason (`| withheld: thin`, `ai_rejected`, `unverifiable`, `over_1500`, `no_sales`). See `AGENTS.md` §7.16.
