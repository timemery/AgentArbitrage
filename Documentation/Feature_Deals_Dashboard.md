...
*   **Filtering:** Users can filter by Keyword, Max Sales Rank, Minimum Profit, Margin, Deal Trust, Seller Trust, and Price Drops.
    ...
    *   **Agent's Choice (Prime Picks):** A special toggle that reads the top AI-selected deals from the cached `prime_picks` table. The AI leverages an asynchronous two-pass pipeline (Pass 1: Smart Floor SQL filtering with a 365-day trend average and Year-Round Velocity Cap, Pass 2: xAI Mastermind evaluation using `grok-4-fast-reasoning` with tiered strategies and seasonal high-rank corrections) and caches the results in the `prime_picks` table for instant reading.
        ...
        *   *Stale pricing (Sept 2026):* the Smart Floor clauses this branch adds include `CURRENT_PRICING_PREDICATE` from `keepa_deals/pricing_version.py` — the exact negation of the predicate `repair_pricing.py` sweeps on. **A cached pick whose `Pricing_Logic_Version` is NULL or below the current one is never rendered**, even though it is sitting in the `prime_picks` table, because the cache is a selection made up to four hours ago and a version bump re-stales every row the moment it deploys. The same predicate gates Pass 1 and evicts stale entries from the cache; see "Stale pricing is never a Prime Pick" in `System_Architecture.md` §3.D. **This applies to Agent's Choice only** — the normal grid still shows stale-priced rows.
