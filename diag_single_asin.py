...
def run_single_asin_diagnostic(asin: str):
    ...
    # More aggressive wait: ensure we have enough tokens before the call
    estimated_cost = 15
    while token_manager.tokens < estimated_cost:
        wait_time = (estimated_cost - token_manager.tokens) * 12 + 12 # 12 seconds per token + buffer
        logger.warning(f"Insufficient tokens for product data fetch. Have: {token_manager.tokens}, Need: {estimated_cost}. Waiting for {wait_time} seconds.")
        time.sleep(wait_time)
        token_manager.sync_tokens()

    product_response, _, _, tokens_left = fetch_product_batch(api_key, [asin], history=1, offers=100) # Request more offers
    ...
    if unique_seller_ids:
        logger.info(f"Found {len(unique_seller_ids)} unique seller IDs: {unique_seller_ids}")
        seller_id_list = list(unique_seller_ids)
        token_manager.request_permission_for_call(estimated_cost=len(seller_id_list) // 100 + 1)
    ...
    # The deal object is minimal as we are only testing the product processing
