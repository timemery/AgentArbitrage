...
su -s /bin/bash -c "cd $APP_DIR && export PYTHONPATH=. && $PURGE_COMMAND" www-data
...
su -s /bin/bash -c "cd $APP_DIR && export PYTHONPATH=. && nohup $WORKER_COMMAND >> $LOG_FILE 2>&1 &" www-data
