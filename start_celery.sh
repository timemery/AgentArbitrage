...
monitor_and_restart() {
    while true; do
        ...
        DIAG_COMMANDS="echo '--- DIAGNOSTIC START ---'; date; echo 'User:'; whoami; echo 'Current Directory:'; pwd; echo 'Python Executable:'; which $VENV_PYTHON; echo 'PYTHONPATH:'; echo \$PYTHONPATH; echo '--- DIAGNOSTIC END ---'"

        # Start the daemons with diagnostics
        echo "Starting Celery worker and beat scheduler daemons with diagnostics..." >> "$MONITOR_LOG_FILE"
        # The --workdir flag is now part of the command itself, which is more robust
        su -s /bin/bash -c "$ENV_SETUP && $DIAG_COMMANDS >> $DIAG_LOG_FILE 2>&1 && nohup $WORKER_COMMAND >> $WORKER_LOG_FILE 2>&1 &" www-data
