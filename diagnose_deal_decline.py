...
def check_live_pipeline_funnel():
    ...
    try:
        ...
        sample_deals = deals_list[:20] if isinstance(deals_list, list) else []
        asins = [d.get('asin') for d in sample_deals if isinstance(d, dict) and d.get('asin')]
        print(f"Sampling {len(asins)} ASINs for processing funnel dry-run...")

        if not asins:
            print(f"{WARNING}No valid ASINs found in candidate deals.{ENDC}")
            return

        # Fetch product batch
        batch_res = fetch_product_batch(api_key, asins, days=365, offers=20, rating=1, history=1)
        data = batch_res[0] if (isinstance(batch_res, tuple) and len(batch_res) > 0 and isinstance(batch_res[0], dict)) else {}
        products = data.get('products', []) if isinstance(data, dict) and 'products' in data else []
        ...
        for p in products:
            ...
            deal_obj = next((d for d in sample_deals if isinstance(d, dict) and d.get('asin') == p.get('asin')), {})
            try:
        ...
        if len(products) > 0 and inferred_sales_pass < len(products) * 0.3:
